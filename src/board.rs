...
impl Board {
    ...
    pub fn create_from_fen(fen: &str) -> Self {
        ...
        board.current_color_turn = match fen_iter.next() {
            Some(white_move) => if white_move == "w" { piece::WHITE } else { piece::BLACK },
            None => panic!("Invalid FEN string"),
        };
    ...
    pub fn swap_current_turn_color(&mut self) {
        self.current_color_turn = if self.current_color_turn == piece::WHITE { piece::BLACK } else { piece::WHITE };
    }
    ...
    fn is_square_controlled(&self, square: usize, attacking_color: u8) -> bool {
        ...
        for offset in pawn_offsets {
            ...
            if new_square >= 0 && new_square < 64 && (new_square as usize % 8).abs_diff(square % 8) == 1 {
                if self.get_square(new_square as usize) == piece::create(piece::PAWN, attacking_color) {
                    return true;
                }
            }
        }
        ...
        for offset in knight_offsets {
            let new_square = square as isize + offset;
            if new_square >= 0 && new_square < 64 && (new_square as usize % 8).abs_diff(square % 8) <= 2 && (new_square as usize / 8).abs_diff(square / 8) <= 2 {
                if self.get_square(new_square as usize) == piece::create(piece::KNIGHT, attacking_color) {
                    return true;
                }
            }
        }
        ...
        for m in pseudo_legal_moves.unwrap() {
            ...
            if m.is_castling() {
                if m.to == 62 {
                    ...
                    if !piece::is_empty(self.get_square(61)) || !piece::is_empty(self.get_square(62)) {
                        continue;
                    }
                    if self.is_square_controlled(61, piece::BLACK) || self.is_square_controlled(60, piece::BLACK) {
                        continue;
                    }
                } else if m.to == 58 {
                    ...
                    if !piece::is_empty(self.get_square(59)) || !piece::is_empty(self.get_square(58)) || !piece::is_empty(self.get_square(57)) {
                        continue;
                    }
                    if self.is_square_controlled(59, piece::BLACK) || self.is_square_controlled(60, piece::BLACK) {
                        continue;
                    }
                } else if m.to == 6 {
                    ...
                    if self.is_square_controlled(5, piece::WHITE) || self.is_square_controlled(4, piece::WHITE) {
                        continue;
                    }
                } else if m.to == 2 {
                    ...
                    if !piece::is_empty(self.get_square(3)) || !piece::is_empty(self.get_square(2)) || !piece::is_empty(self.get_square(1)) {
                        continue;
                    }
                    if self.is_square_controlled(3, piece::WHITE) || self.is_square_controlled(4, piece::WHITE) {
                        continue;
                    }
                }
            }
